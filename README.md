...
- **Measured deltas, not impressions.** Every step is an experiment with a number attached (recall, precision, cost, latency). When a fancier pattern does not beat a simpler one on the eval set, that is a result too, and the simpler one stays.
