...
The corpus is fetched from pinned upstream refs, so the build is reproducible. (The refs in `config.py` are placeholders at this stage, so `make index` does not yet run end to end: the fetch fails against the placeholder refs until they are pinned to real commits.)
